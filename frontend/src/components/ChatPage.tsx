...
import zipoBotIcon from '../../assets/zipo_belang.png';
import zipoIcon from '../../assets/zipo_semi_transparent.png';
    ...
    <div className="h-screen flex bg-slate-100 overflow-hidden"> {/* Main container for the chat and canvas layout. */}
      <div className={`flex flex-col bg-white border-r border-slate-200 transition-all duration-500 ease-in-out flex-shrink-0 overflow-hidden ${chatMinimized ? 'w-0' : 'w-full md:w-1/2 lg:w-1/3'}`}> {/* Chat panel container. */}
        <div className={`w-full h-full flex flex-col transition-opacity duration-300 ${chatMinimized ? 'opacity-0' : 'opacity-100'}`}> {/* Inner container for chat content, handles opacity during minimization. */}
            <div className="p-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between"> {/* Chat header. */}
                <div>
                  <h2 className="text-lg font-bold text-slate-800">Learning Session</h2>
                  <p className="text-xs text-slate-500 truncate max-w-xs">{isNew ? 'New Session' : sessionId}</p>
                </div>
                <button onClick={toggleChatMinimize} className="p-2 text-slate-400 hover:text-slate-600 transition-colors" title="Hide Chat">
                  <ChevronLeft size={20} />
                </button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-4"> {/* Chat messages display area. */}
              ...
              {messages.map((message) => (
                <div key={message.id} className={`flex flex-col ${message.type === 'user' ? 'items-end' : 'items-start'}`}> {/* Individual message container. */}
                    <div className={`flex gap-3 w-full ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}> {/* Message bubble and avatar alignment. */}
                        {message.type === 'ai' && <div className="w-8 h-8 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full flex items-center justify-center flex-shrink-0"><img src={zipoIcon} alt="Bot icon" className="w-6 h-6" /></div>} {/* AI avatar. */}
                        <div className={`max-w-xs lg:max-w-md px-4 py-3 rounded-2xl ${message.type === 'user' ? 'bg-blue-500 text-white' : 'bg-slate-100 text-slate-800'}`}> {/* Message bubble styling. */}
                            ...
                            <span className={`text-xs opacity-70 mt-1 block ${message.type === 'user' ? 'text-blue-100' : 'text-slate-500'}`}>{message.timestamp.toLocaleTimeString()}</span> {/* Message timestamp. */}
                        </div>
                        {message.type === 'user' && <div className="w-8 h-8 bg-slate-200 rounded-full flex items-center justify-center flex-shrink-0"><User size={16} className="text-slate-600" /></div>} {/* User avatar. */}
                    </div>
                </div>
              ))}
              {isLoading && (
                <div className="flex gap-3 justify-start"> {/* Loading indicator for AI response. */}
                  <div className="w-8 h-8 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full flex items-center justify-center flex-shrink-0"><img src={zipoIcon} alt="Bot icon" className="w-6 h-6" /></div>
                  <div className="bg-slate-100 rounded-2xl px-4 py-3 flex items-center"><div className="flex space-x-1"><div className="w-2 h-2 bg-slate-400 rounded-full animate-bounce"></div><div className="w-2 h-2 bg-slate-400 rounded-full animate-bounce" style={{ animationDelay: '0.1s' }}></div><div className="w-2 h-2 bg-slate-400 rounded-full animate-bounce" style={{ animationDelay: '0.2s' }}></div></div></div>
                </div>
              )}
              <div ref={messagesEndRef} /> {/* Ref for auto-scrolling. */}
            </div>
            <div className="p-4 border-t border-slate-200 bg-slate-50"> {/* Message input area. */}
              ...
              {isListening && (
                <div className="relative mb-2 animate-in fade-in slide-in-from-bottom-2 duration-300"> {/* Live speech recognition indicator. */}
                  <div className="absolute -inset-0.5 bg-gradient-to-r from-blue-500 to-purple-600 rounded-xl blur-md opacity-60"></div>
                  <div className="relative px-4 py-2 bg-slate-800 rounded-lg leading-none flex items-center">
                    <div className="flex items-center gap-2 text-purple-300">
                        <div className="flex gap-0.5 items-center h-4">
                            <span className="w-1 h-1 bg-purple-400 rounded-full animate-pulse" style={{ animationDelay: '0s' }}></span>
                            <span className="w-1 h-2 bg-purple-400 rounded-full animate-pulse" style={{ animationDelay: '0.1s' }}></span>
                            <span className="w-1 h-3 bg-purple-400 rounded-full animate-pulse" style={{ animationDelay: '0.2s' }}></span>
                            <span className="w-1 h-2 bg-purple-400 rounded-full animate-pulse" style={{ animationDelay: '0.3s' }}></span>
                            <span className="w-1 h-1 bg-purple-400 rounded-full animate-pulse" style={{ animationDelay: '0.4s' }}></span>
                        </div>
              ...
              <div className="flex items-center gap-3"> {/* Input field and action buttons. */}
                ...
                <div className="flex-1 relative">
                  <textarea
                    ...
                    className="w-full p-3 pr-10 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
                    rows={1}
                    disabled={isLoading}
                  />
                </div>
                <button 
                  ...
                  className={`p-3 transition-all duration-300 rounded-full disabled:opacity-50 disabled:cursor-not-allowed ${
                    isListening 
                      ? 'bg-purple-600 text-white shadow-lg shadow-purple-500/50 ring-4 ring-purple-500/30' 
                ...
                <button id="send-button" onClick={() => handleSendMessage()} disabled={!(isListening ? transcript : inputValue).trim() || isLoading} className="p-3 bg-blue-500 text-white rounded-xl hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all">
                  <Send size={20} />
                </button>
              </div>
            </div>
      ...
      <div className={`bg-slate-100 relative flex flex-col transition-all duration-500 ease-in-out flex-1`}> {/* Canvas panel container. */}
        <div className="p-4 bg-white border-b border-slate-200 flex items-center gap-4 flex-shrink-0"> {/* Canvas header. */}
          <div className="flex items-center gap-3 min-w-0">
            {chatMinimized && <button onClick={toggleChatMinimize} className="p-2 text-slate-500 hover:bg-slate-100 rounded-lg flex-shrink-0" title="Show Chat"><ChevronRight size={20} /></button>}
            <h3 className="text-lg font-semibold text-slate-800 truncate">Visual Learning Canvas</h3>
          </div>
          <div className="flex items-center flex-shrink-0 gap-2 ml-auto"> {/* Canvas control buttons. */}
            <button 
              ...
              className={`
                ...
                ${isNew
                  ? 'bg-slate-100 text-slate-400 cursor-not-allowed opacity-50 blur-[1px]'
                  : 'bg-purple-100 text-purple-600 hover:bg-purple-200 shadow-[0_0_15px_rgba(168,85,247,0.5)] hover:shadow-[0_0_20px_rgba(168,85,247,0.7)]'
                }
              `}
            ...
            <button onClick={handlePlayPause} disabled={commandQueue.length === 0 && !isPlaying} className={`flex items-center space-x-2 px-3 py-2 rounded-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed ${isPlaying ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>
              {isPlaying ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />}
              <span className="hidden md:inline">{isPlaying ? 'Pause' : 'Play'}</span>
            </button>
            <button onClick={handleReset} className="flex items-center space-x-2 px-3 py-2 bg-slate-200 hover:bg-slate-300 text-slate-700 rounded-lg transition-all">
              <RotateCcw className="w-4 h-4" />
              <span className="hidden md:inline">Reset</span>
            </button>
            <button onClick={toggleChatMinimize} className="p-2 text-slate-500 hover:bg-slate-200 rounded-lg transition-all" title={chatMinimized ? "Show Chat" : "Focus Mode"}>
              {chatMinimized ? <Minimize2 className="w-5 h-5" /> : <Maximize2 className="w-5 h-5" />}
            </button>
          </div>
        </div>
        <div ref={canvasContainerRef} className="flex-1 overflow-hidden bg-gradient-to-br from-blue-50 to-purple-50"> {/* Konva canvas rendering area. */}
        ...
        {currentTranscript && (
          <div className="absolute bottom-20 left-4 right-4 bg-black/80 backdrop-blur-sm rounded-2xl p-6 text-white shadow-2xl border border-white/20 animate-in slide-in-from-bottom-4 duration-300"> {/* Live AI transcript display. */}
            <div className="flex items-start gap-4">
              <div className="w-12 h-12 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full flex items-center justify-center flex-shrink-0"><img src={zipoIcon} alt="Bot icon" className="w-7 h-7" /></div>
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2"><span className="text-sm font-semibold text-blue-300">Zipo</span><div className="flex items-center gap-1"><div className="w-2 h-2 bg-red-500 rounded-full animate-pulse"></div><span className="text-xs text-white/70">LIVE</span></div></div>
                <p className="text-white leading-relaxed text-lg font-medium">{currentTranscript}</p>
              </div>
            </div>
          </div>
        )}
        <div className="p-4 bg-white border-t border-slate-200 flex-shrink-0"> {/* Lesson progress bar. */}
          ...
          <div className="w-full bg-gray-200 rounded-full h-2"><div className="bg-gradient-to-r from-blue-500 to-purple-600 h-2 rounded-full transition-all duration-300" style={{ width: `${lessonSteps.total > 0 ? (lessonSteps.current / lessonSteps.total) * 100 : 0}%` }}></div></div>
        </div>
